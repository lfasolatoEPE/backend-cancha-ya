...
export class NotifsService {
  ...
  async subscribe(userId: string, channel: 'email'|'push', address: string) {
    if (!NOTIFS_ENABLED) throw new Error('Notificaciones deshabilitadas');
  ...
  async sendEmailToUser(userId: string, subject: string, html: string, text?: string) {
    if (!NOTIFS_ENABLED) {
      console.warn('[NOTIFS] disabled → sendEmailToUser no-op');
      return;
    }
    ...
    for (const s of subs) {
      await enqueue(JOB_SEND_MAIL, { to: s.address, subject, html, text }, { attempts: 5, backoff: { type: 'exponential', delay: 5000 } });
      await this.log('custom', userId, { to: s.address, subject });
    }
  }

  async scheduleReservaReminder(userId: string, payload: { fechaISO: string; club: string; cancha: string; reservaId: string }, runAtISO: string) {
    if (!NOTIFS_ENABLED) {
      console.warn('[NOTIFS] disabled → scheduleReservaReminder no-op');
      return;
    }
    ...
    await enqueue(JOB_RESERVA_RECORDATORIO, { userId, payload }, { delay: delayMs, attempts: 5, backoff: { type: 'exponential', delay: 5000 } });
  }

  private async log(kind: NotificationLog['kind'], userId: string, payload: any, status: NotificationLog['status'] = 'SENT') {
